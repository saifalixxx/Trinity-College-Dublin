...
fp_add:
  PUSH    {R5-R12, LR}                   @ Push all registers  from R5 to R12

  MOV R5, R0                             @ x = a
  MOV R6, R1                             @ y = b
  MOV R0, R5                             @ a = x

  BL fp_exp                              @ Call function

  MOV R7, R0                             @ z = a
  MOV R0, R5                             @ a = x

  BL fp_frac                             @ Call function
  ...
  SUB R11, R9, R7                        @ f = h - z
  ADD R7, R7, R11                        @ z = z + f
  MOV R8, R8, ASR R11                    @ l = l shift both negative and positive cases f
   B  terminateAdd

addGreater :
  SUB R11, R7, R9                        @ f = z - h
  ADD R9, R9, R11                        @ h = h + f
  MOV R10, R10, ASR R11                  @ z = z shift both negative and positive cases f

terminateAdd :

addEqual :
  ADD R12, R8, R10                       @ j = l + z
  MOV R0, R12                            @ a = j
  MOV R1, R7                             @ b = z
  BL fp_enc                              @ Call function

  POP     {R5-R12, PC}                   @ Pop all registers from R5 to R12              
